...
class test_picture(unittest.TestCase):
    ...
    def test_register_inference_result(self):
        ...
        result = asyncio.run(Nachet.register_inference_result(self.cursor,self.user_id,self.inference, picture_id, model_id))
    ...
    def test_upload_picture_known(self):
        ...
        picture_id = asyncio.run(Nachet.upload_picture_known(self.cursor, self.user_id, self.pic_encoded,self.container_client, self.seed_id, picture_set_id))
        self.assertTrue(validator.is_valid_uuid(picture_id))
    ...
    def test_upload_picture_known_error_user_not_found(self):
        ...
        with self.assertRaises(Nachet.user.UserNotFoundError):
            asyncio.run(Nachet.upload_picture_known(self.cursor, uuid.uuid4(), self.pic_encoded,self.container_client, self.seed_id, picture_set_id))
    ...
    def test_upload_picture_known_connection_error(self):
        ...
        with self.assertRaises(Exception):
            asyncio.run(Nachet.upload_picture_known(mock_cursor, self.user_id, self.pic_encoded,self.container_client, self.seed_id, picture_set_id))
...
class test_picture_set(unittest.TestCase):
    ...
    def test_find_validated_pictures(self):
        ...
        self.assertEqual(len(asyncio.run(Nachet.find_validated_pictures(self.cursor, str(self.user_id), str(self.picture_set_id)))), 0, "No validated pictures should be found")
        ...
        for picture_id in self.pictures_ids :
            ...
            inference = asyncio.run(Nachet.register_inference_result(self.cursor,self.user_id,inference_copy, picture_id, "test_model_id"))
            inferences.append(inference)
        
        asyncio.run(Nachet.new_perfect_inference_feeback(self.cursor, inferences[0]["inference_id"], self.user_id, [box["box_id"] for box in inferences[0]["boxes"]]))
        
        self.assertEqual(len(asyncio.run(Nachet.find_validated_pictures(self.cursor, str(self.user_id), str(self.picture_set_id)))), 1, "One validated pictures should be found")
        
        asyncio.run(Nachet.new_perfect_inference_feeback(self.cursor, inferences[1]["inference_id"], self.user_id, [box["box_id"] for box in inferences[1]["boxes"]]))
        asyncio.run(Nachet.new_perfect_inference_feeback(self.cursor, inferences[2]["inference_id"], self.user_id, [box["box_id"] for box in inferences[2]["boxes"]]))

        self.assertEqual(len(asyncio.run(Nachet.find_validated_pictures(self.cursor, str(self.user_id), str(self.picture_set_id)))), 3, "One validated pictures should be found")
    ...
    def test_find_validated_pictures_error_user_not_found(self):
        ...
        with self.assertRaises(Nachet.user.UserNotFoundError):
            asyncio.run(Nachet.find_validated_pictures(self.cursor, str(uuid.uuid4()), str(self.picture_set_id)))
    ...
    def test_find_validated_pictures_error_connection_error(self):
        ...
        mock_cursor.fetchone.side_effect = Exception("Connection error")
        with self.assertRaises(Exception):
            asyncio.run(Nachet.find_validated_pictures(mock_cursor, str(self.user_id), str(self.picture_set_id)))
    ...
    def test_find_validated_pictures_error_not_owner(self):
        ...
        with self.assertRaises(Nachet.UserNotOwnerError):
            asyncio.run(Nachet.find_validated_pictures(self.cursor, str(not_owner_user_id), str(self.picture_set_id)))
    ...
    def test_delete_picture_set_with_archive(self):
        ...
        asyncio.run(Nachet.new_perfect_inference_feeback(self.cursor, inferences[1]["inference_id"], self.user_id, [box["box_id"] for box in inferences[1]["boxes"]]))
        asyncio.run(Nachet.new_perfect_inference_feeback(self.cursor, inferences[2]["inference_id"], self.user_id, [box["box_id"] for box in inferences[2]["boxes"]]))
        validated_pictures = asyncio.run(Nachet.find_validated_pictures(self.cursor, str(self.user_id), str(self.picture_set_id)))
        ...
        dev_picture_set_id = asyncio.run(Nachet.delete_picture_set_with_archive(self.cursor, str(self.user_id), str(self.picture_set_id), self.container_client))
        print("moved")
        ...
        for picture_id in validated_pictures :
            ...
            with self.assertRaises(Exception):
                asyncio.run(datastore.azure_storage.get_blob(self.container_client, blob_name))
        
            blob_name = "{}/{}/{}.png".format(str(self.user_id), self.folder_name, str(picture_id))
            print("test: " + blob_name)
            blob = asyncio.run(datastore.azure_storage.get_blob(self.dev_container_client, blob_name))
            self.assertEqual(blob, self.pic_encoded)
        
        # TEAR DOWN
        # Delete the user folder in the blob storage
        asyncio.run(datastore.azure_storage.delete_folder(self.dev_container_client, str(dev_picture_set_id)))
        asyncio.run(datastore.azure_storage.delete_folder(self.dev_container_client, str(self.user_id)))
    ...
    def test_delete_picture_set_with_archive_error_user_not_found(self):
        ...
        """
        with self.assertRaises(Nachet.user.UserNotFoundError):
            asyncio.run(Nachet.delete_picture_set_with_archive(self.cursor, str(uuid.uuid4()), str(self.picture_set_id), self.container_client))
    ...
    def test_delete_picture_set_with_archive_error_connection_error(self):
        ...
        mock_cursor.fetchone.side_effect = Exception("Connection error")
        with self.assertRaises(Exception):
            asyncio.run(Nachet.delete_picture_set_with_archive(mock_cursor, str(self.user_id), str(self.picture_set_id), self.container_client))
    ...
    def test_delete_picture_set_with_archive_error_picture_set_not_found(self):
        ...
        """
        with self.assertRaises(Nachet.picture.PictureSetNotFoundError):
            asyncio.run(Nachet.delete_picture_set_with_archive(self.cursor, str(self.user_id), str(uuid.uuid4()), self.container_client))
    ...
    def test_delete_picture_set_with_archive_error_not_owner(self):
        ...
        with self.assertRaises(Nachet.UserNotOwnerError):
            asyncio.run(Nachet.delete_picture_set_with_archive(self.cursor, str(not_owner_user_id), str(self.picture_set_id), self.container_client))
    ...
    def test_delete_picture_set_with_archive_error_default_folder(self):
        ...
        with self.assertRaises(Nachet.picture.PictureSetDeleteError):
            asyncio.run(Nachet.delete_picture_set_with_archive(self.cursor, str(self.user_id), str(general_folder_id), self.container_client))
...
class test_feedback(unittest.TestCase):
    def setUp(self):
        ...
        picture_id = asyncio.run(Nachet.upload_picture_unknown(self.cursor, self.user_id, self.pic_encoded,self.container_client))
        model_id = "test_model_id"
        self.registered_inference = asyncio.run(Nachet.register_inference_result(self.cursor,self.user_id,self.inference, picture_id, model_id))
    ...
    def test_new_perfect_inference_feedback(self):
        ...
        """
        asyncio.run(Nachet.new_perfect_inference_feeback(self.cursor, self.inference_id, self.user_id, self.boxes_id))
        for i in range(len(self.boxes_id)) :
            object = Nachet.inference.get_inference_object(self.cursor, self.boxes_id[i])
    ...
    def test_new_perfect_inference_feedback_error_verified_inference(self):
        ...
        """
        asyncio.run(Nachet.new_perfect_inference_feeback(self.cursor, self.inference_id, self.user_id, self.boxes_id))
        self.assertTrue(Nachet.inference.is_inference_verified(self.cursor, self.inference_id))
    ...
    def test_new_perfect_inference_feedback_error_inference_not_found(self):
        ...
        with self.assertRaises(Nachet.inference.InferenceNotFoundError):
            asyncio.run(Nachet.new_perfect_inference_feeback(self.cursor, str(uuid.uuid4()), self.user_id, self.boxes_id))
    ...
    def test_new_perfect_inference_feedback_error_inference_object_not_found(self):
        ...
        with self.assertRaises(Nachet.inference.InferenceObjectNotFoundError):
            asyncio.run(Nachet.new_perfect_inference_feeback(self.cursor, self.inference_id, self.user_id, [self.boxes_id[0], str(uuid.uuid4())]))
    ...
    def test_new_perfect_inference_feedback_error_user_not_found(self):
        ...
        """
        with self.assertRaises(Nachet.user.UserNotFoundError):
            asyncio.run(Nachet.new_perfect_inference_feeback(self.cursor, self.inference_id, str(uuid.uuid4()), self.boxes_id))
    ...
    def test_new_perfect_inference_feedback_connection_error(self):
        ...
        mock_cursor.fetchone.side_effect = Exception("Connection error")
        with self.assertRaises(Exception):
            asyncio.run(Nachet.new_perfect_inference_feeback(mock_cursor, self.inference_id, self.user_id, self.boxes_id))
    ...
    def test_new_correction_inference_feedback(self):
        ...
        asyncio.run(Nachet.new_correction_inference_feedback(self.cursor, self.registered_inference, 1))
        for i in range(len(self.boxes_id)) :
            object = Nachet.inference.get_inference_object(self.cursor, self.boxes_id[i])
            # verified_id must be equal to top_id
            self.assertEqual(str(object[4]), self.top_id[i])
    ...
    def test_new_correction_inference_feedback_new_guess(self):
        ...
        asyncio.run(Nachet.new_correction_inference_feedback(self.cursor, self.registered_inference, 1))
        for i in range(len(self.boxes_id)) :
            object_db = Nachet.inference.get_inference_object(self.cursor, self.boxes_id[i])
            # verified_id must be equal to top_id
    ...
    def test_new_correction_inference_feedback_box_edited(self):
        ...
        asyncio.run(Nachet.new_correction_inference_feedback(self.cursor, self.registered_inference, 1))
        for box in self.registered_inference["boxes"] :
            object_db = Nachet.inference.get_inference_object(self.cursor, box["box_id"])
    ...
    def test_new_correction_inference_feedback_not_guess(self):
        ...
        for i in range(len(self.boxes_id)) :
            ...
            new_top_id = Nachet.inference.get_seed_object_id(self.cursor,self.unreal_seed_id,object_db[0])

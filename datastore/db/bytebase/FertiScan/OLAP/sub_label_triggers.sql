...
BEGIN
    IF (TG_OP = 'INSERT') THEN
        ...
        SELECT sub_type.type_en INTO type_str FROM "fertiscan_0.0.15".sub_type WHERE sub_type.id = NEW.sub_type_id;
...
DROP TRIGGER IF EXISTS sub_label_creation ON "fertiscan_0.0.15".sub_label;
CREATE TRIGGER sub_label_creation
AFTER INSERT ON "fertiscan_0.0.15".sub_label
FOR EACH ROW
...
BEGIN
    IF (TG_OP = 'DELETE') THEN
        ...
        -- FIND THE SUB_TYPE TO GET THE COLUMN IDENTIFIER
        SELECT sub_type.type_en INTO type_str FROM "fertiscan_0.0.15".sub_type WHERE sub_type.id = OLD.sub_type_id;
        IF (type_str ILIKE 'test%') THEN
...
DROP TRIGGER IF EXISTS sub_label_deletion ON "fertiscan_0.0.15".sub_label;
CREATE TRIGGER sub_label_deletion
AFTER DELETE ON "fertiscan_0.0.15".sub_label
FOR EACH ROW

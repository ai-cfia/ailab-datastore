...
# import datastore.FertiScan.__init__ as fertiscan_datastore
import datastore.Nachet.__init__ as nachet_datastore
...
class test_ml_structure(unittest.TestCase):
    ...
    def test_import_ml_structure_from_json(self):
        ...
        """
        asyncio.run(nachet_datastore.import_ml_structure_from_json_version(self.cursor,self.ml_dict))
    ...
    def test_get_ml_structure(self):
        ...
        ml_structure= asyncio.run(nachet_datastore.get_ml_structure(self.cursor))
    ...
    def test_get_ml_structure_eeror(self):
        ...
        with self.assertRaises(nachet_datastore.MLRetrievalError):
            asyncio.run(nachet_datastore.get_ml_structure(mock_cursor))

# TODO: fix datastore.get_user_container_client args
...
#         result = asyncio.run(nachet_datastore.register_inference_result(self.cursor,self.user_id,self.inference, picture_id, model_id))
...
#         """
#         picture_set_id = asyncio.run(datastore.create_picture_set(self.cursor, self.container_client, 0, self.user_id))
#         picture_id = asyncio.run(nachet_datastore.upload_picture_known(self.cursor, self.user_id, self.pic_encoded,self.container_client, self.seed_id, picture_set_id))
#         self.assertTrue(validator.is_valid_uuid(picture_id))
...
#         picture_set_id = asyncio.run(datastore.create_picture_set(self.cursor, self.container_client, 0, self.user_id))
#         with self.assertRaises(datastore.user.UserNotFoundError):
#             asyncio.run(nachet_datastore.upload_picture_known(self.cursor, uuid.uuid4(), self.pic_encoded,self.container_client, self.seed_id, picture_set_id))
...
#         picture_set_id = asyncio.run(datastore.create_picture_set(self.cursor, self.container_client, 0, self.user_id))
#         with self.assertRaises(Exception):
#             asyncio.run(nachet_datastore.upload_picture_known(mock_cursor, self.user_id, self.pic_encoded,self.container_client, self.seed_id, picture_set_id))
...
#         """
#         picture_set_id = asyncio.run(datastore.create_picture_set(self.cursor, self.container_client, 0, self.user_id))
#         with self.assertRaises(nachet_datastore.seed.SeedNotFoundError):
...
#         self.assertEqual(len(asyncio.run(nachet_datastore.find_validated_pictures(self.cursor, str(self.user_id), str(self.picture_set_id)))), 0, "No validated pictures should be found")
...
#             inference = asyncio.run(nachet_datastore.register_inference_result(self.cursor,self.user_id,inference_copy, picture_id, "test_model_id"))
#             inferences.append(inference)
        
#         asyncio.run(nachet_datastore.new_perfect_inference_feeback(self.cursor, inferences[0]["inferenceId"], self.user_id, [box["boxId"] for box in inferences[0]["boxes"]]))
        
#         self.assertEqual(len(asyncio.run(nachet_datastore.find_validated_pictures(self.cursor, str(self.user_id), str(self.picture_set_id)))), 1, "One validated pictures should be found")
        
#         asyncio.run(nachet_datastore.new_perfect_inference_feeback(self.cursor, inferences[1]["inferenceId"], self.user_id, [box["boxId"] for box in inferences[1]["boxes"]]))
#         asyncio.run(nachet_datastore.new_perfect_inference_feeback(self.cursor, inferences[2]["inferenceId"], self.user_id, [box["boxId"] for box in inferences[2]["boxes"]]))

#         self.assertEqual(len(asyncio.run(nachet_datastore.find_validated_pictures(self.cursor, str(self.user_id), str(self.picture_set_id)))), 3, "One validated pictures should be found")
...
#         with self.assertRaises(datastore.user.UserNotFoundError):
#             asyncio.run(nachet_datastore.find_validated_pictures(self.cursor, str(uuid.uuid4()), str(self.picture_set_id)))
...
#         mock_cursor.fetchone.side_effect = Exception("Connection error")
#         with self.assertRaises(Exception):
#             asyncio.run(nachet_datastore.find_validated_pictures(mock_cursor, str(self.user_id), str(self.picture_set_id)))
...
#             asyncio.run(nachet_datastore.find_validated_pictures(self.cursor, str(self.user_id), str(uuid.uuid4())))
...
#             asyncio.run(nachet_datastore.find_validated_pictures(self.cursor, str(not_owner_user_id), str(self.picture_set_id)))
...
#         asyncio.run(nachet_datastore.new_perfect_inference_feeback(self.cursor, inferences[1]["inferenceId"], self.user_id, [box["boxId"] for box in inferences[1]["boxes"]]))
#         asyncio.run(nachet_datastore.new_perfect_inference_feeback(self.cursor, inferences[2]["inferenceId"], self.user_id, [box["boxId"] for box in inferences[2]["boxes"]]))
#         validated_pictures = asyncio.run(nachet_datastore.find_validated_pictures(self.cursor, str(self.user_id), str(self.picture_set_id)))
...
#         dev_picture_set_id = asyncio.run(nachet_datastore.delete_picture_set_with_archive(self.cursor, str(self.user_id), str(self.picture_set_id), self.container_client))
...
#             blob_name = "{}/{}.png".format(self.folder_name, picture_id)
#             with self.assertRaises(Exception):
#                 asyncio.run(datastore.azure_storage.get_blob(self.container_client, blob_name))
        
#             blob_name = "{}/{}/{}.png".format(self.user_id, self.folder_name, str(picture_id))
...
#         """
#         with self.assertRaises(datastore.user.UserNotFoundError):
#             asyncio.run(nachet_datastore.delete_picture_set_with_archive(self.cursor, str(uuid.uuid4()), str(self.picture_set_id), self.container_client))
...
#         mock_cursor.fetchone.side_effect = Exception("Connection error")
#         with self.assertRaises(Exception):
#             asyncio.run(nachet_datastore.delete_picture_set_with_archive(mock_cursor, str(self.user_id), str(self.picture_set_id), self.container_client))
...
#         """
#         with self.assertRaises(datastore.picture.PictureSetNotFoundError):
#             asyncio.run(nachet_datastore.delete_picture_set_with_archive(self.cursor, str(self.user_id), str(uuid.uuid4()), self.container_client))
...
#         with self.assertRaises(datastore.UserNotOwnerError):
#             asyncio.run(nachet_datastore.delete_picture_set_with_archive(self.cursor, str(not_owner_user_id), str(self.picture_set_id), self.container_client))
...
#         general_folder_id = datastore.user.get_default_picture_set(self.cursor, self.user_id)
#         with self.assertRaises(datastore.picture.PictureSetDeleteError):
#             asyncio.run(nachet_datastore.delete_picture_set_with_archive(self.cursor, str(self.user_id), str(general_folder_id), self.container_client))

# TODO: fix fertiscan_datastore.register_analysis args
...
#         analysis = asyncio.run(fertiscan_datastore.register_analysis(self.cursor, self.container_client,self.analysis_dict,self.pic_encoded,"","General"))

...
from datastore.db.queries import inspection, ingredient, label,metric,nutrients,organization,specification,sub_label,user, picture
...
class test_inspection_export(unittest.TestCase):
    def setUp(self):
        ...
        self.picture_set_id = picture.new_picture_set(self.cursor, json.dumps({}), self.user_id)
    ...
    def test_perfect_inspection(self):
        ...
        inspection_dict = inspection.new_inspection_with_label_info(self.cursor, self.user_id,self.picture_set_id,formatted_analysis)
        ...
        data = metadata.build_inspection_export(self.cursor, str(inspection_id), label_information_id)
        ...
        self.assertListEqual(inspection_dict['guaranteed_analysis'], data['guaranteed_analysis'])

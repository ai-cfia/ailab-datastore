...
CREATE OR REPLACE FUNCTION "fertiscan_0.0.14".after_org_info_delete_location_trig()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    IF OLD.location_id IS NOT NULL THEN
        BEGIN
            DELETE FROM "fertiscan_0.0.14".location
...
DROP TRIGGER IF EXISTS after_organization_information_delete_location ON "fertiscan_0.0.14".organization_information;
CREATE TRIGGER after_organization_information_delete_location
AFTER DELETE ON "fertiscan_0.0.14".organization_information
FOR EACH ROW
EXECUTE FUNCTION "fertiscan_0.0.14".after_org_info_delete_location_trig();

-- Function to delete an inspection and related data
CREATE OR REPLACE FUNCTION "fertiscan_0.0.14".delete_inspection(
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM "fertiscan_0.0.14".inspection 
    ...
    WITH deleted_inspection AS (
        DELETE FROM "fertiscan_0.0.14".inspection
        WHERE id = p_inspection_id
        RETURNING *
    )
...
CREATE OR REPLACE FUNCTION "fertiscan_0.0.14".after_insp_delete_cleanup_trig()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    IF OLD.sample_id IS NOT NULL THEN
        DELETE FROM "fertiscan_0.0.14".sample
        WHERE id = OLD.sample_id;
    END IF;

    IF OLD.label_info_id IS NOT NULL THEN
        DELETE FROM "fertiscan_0.0.14".label_information
...
DROP TRIGGER IF EXISTS after_inspection_delete_cleanup ON "fertiscan_0.0.14".inspection;
CREATE TRIGGER after_inspection_delete_cleanup
AFTER DELETE ON "fertiscan_0.0.14".inspection
FOR EACH ROW
EXECUTE FUNCTION "fertiscan_0.0.14".after_insp_delete_cleanup_trig();

-- Trigger function to handle after label_information delete for organization_information deletion
CREATE OR REPLACE FUNCTION "fertiscan_0.0.14".after_label_info_delete_org_info_trig()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    BEGIN
        DELETE FROM "fertiscan_0.0.14".organization_information
...
DROP TRIGGER IF EXISTS after_label_information_delete_organization_information ON "fertiscan_0.0.14".label_information;
CREATE TRIGGER after_label_information_delete_organization_information
AFTER DELETE ON "fertiscan_0.0.14".label_information
FOR EACH ROW
EXECUTE FUNCTION "fertiscan_0.0.14".after_label_info_delete_org_info_trig();

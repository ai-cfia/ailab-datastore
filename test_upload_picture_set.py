...
class test_upload_picture_set(unittest.TestCase):
    ...
    def test_upload_picture_set_blob(self):
        ...
        """
        container_client = asyncio.run(datastore.get_user_container_client(self.user_id,"test-user"))
        #new_container_client = ContainerClient.from_container_url(container_url=url+sas)
        zoom_level = 1.0
        nb_seeds = 1
        picture_set_id = upload_picture_set.upload_picture_set(self.cursor, container_client, self.pictures, self.user_id, self.seed_name, zoom_level, nb_seeds)
        self.assertTrue(picture_query.is_a_picture_set_id(self.cursor, picture_set_id))
        ...
        for blob in blobs_list:
            ...
            if tags["picture_set_uuid"] == str(picture_set_id):
                nb+=1
        self.assertTrue(nb == len(self.pictures)+1,f"nb={nb} len={len(self.pictures)}")     

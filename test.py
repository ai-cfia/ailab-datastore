...
class test_pictures_functions(unittest.TestCase):
    def setUp(self):
        ...
        self.pic_path = 'img/test_image.tiff'
        self.picture_set = picture_set_data.build_picture_set(self.user_id, 1)
        self.picture = picture_data.build_picture(self.pic_path, self.seed_name, 1, 1.0)
        
    def tearDown(self):
        db.end_query(self.con, self.cursor)
        
    def test_new_picture_set(self):
        picture_set_id = picture.new_picture_set(self.cursor, self.picture_set, self.user_id)
    ...
    def test_new_picture(self):
        ...
        picture_id = picture.new_picture(self.cursor, self.picture, picture_set_id, seed_id)

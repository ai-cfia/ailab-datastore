...
DROP TRIGGER IF EXISTS after_organization_delete_main_location ON "fertiscan_0.0.19".organization;
drop FUNCTION IF EXISTS "fertiscan_0.0.19".after_org_delete_location_trig();
CREATE OR REPLACE FUNCTION "fertiscan_0.0.19".after_org_delete_location_trig()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
BEGIN
    IF OLD.main_location_id IS NOT NULL THEN
        BEGIN
            DELETE FROM "fertiscan_0.0.19".location
...
AFTER DELETE ON "fertiscan_0.0.19".organization
FOR EACH ROW
EXECUTE FUNCTION "fertiscan_0.0.19".after_org_delete_location_trig();

-- Function to delete an inspection and related data
CREATE OR REPLACE FUNCTION "fertiscan_0.0.19".delete_inspection(
...
BEGIN
    IF NOT EXISTS (
        SELECT 1
        FROM "fertiscan_0.0.19".inspection 
    ...
    WITH deleted_inspection AS (
        DELETE FROM "fertiscan_0.0.19".inspection
        WHERE id = p_inspection_id
        RETURNING *
    )
...
CREATE OR REPLACE FUNCTION "fertiscan_0.0.19".after_insp_delete_cleanup_trig()
RETURNS TRIGGER
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    IF OLD.sample_id IS NOT NULL THEN
        DELETE FROM "fertiscan_0.0.19".sample
        WHERE id = OLD.sample_id;
    END IF;

    IF OLD.label_info_id IS NOT NULL THEN
        DELETE FROM "fertiscan_0.0.19".label_information
    ...
    IF OLD.fertilizer_id IS NOT NULL THEN
        ...
        if newest_inspection_id IS NOT NULL THEN
            UPDATE "fertiscan_0.0.19".fertilizer
        ...
        --This was the only inspection for the fertilizer so we delete it
            DELETE FROM "fertiscan_0.0.19".fertilizer
            WHERE id = OLD.fertilizer_id;
        end if;
    END IF;
...
DROP TRIGGER IF EXISTS after_inspection_delete_cleanup ON "fertiscan_0.0.19".inspection;
CREATE TRIGGER after_inspection_delete_cleanup
AFTER DELETE ON "fertiscan_0.0.19".inspection
FOR EACH ROW
EXECUTE FUNCTION "fertiscan_0.0.19".after_insp_delete_cleanup_trig();

...
class test_picture(unittest.TestCase):
    ...
    def test_get_picture_inference(self):
        ...
        inference = asyncio.run(datastore.register_inference_result(self.cursor,self.user_id,self.inference, picture_id, "test_model_id"))

        picture_inference = asyncio.run(datastore.get_picture_inference(self.cursor, str(self.user_id), str(picture_id)))
    ...
    def test_get_picture_inference_error_user_not_found(self):
        ...
        with self.assertRaises(datastore.user.UserNotFoundError):
            asyncio.run(datastore.get_picture_inference(self.cursor, str(uuid.uuid4()), str(picture_id)))
    ...
    def test_get_picture_inference_error_connection_error(self):
        ...
        with self.assertRaises(Exception):
            asyncio.run(datastore.get_picture_inference(mock_cursor, str(self.user_id), str(picture_id)))
    ...
    def test_get_picture_inference_error_picture_not_found(self):
        ...
        with self.assertRaises(datastore.picture.PictureNotFoundError):
            asyncio.run(datastore.get_picture_inference(self.cursor, str(self.user_id), str(uuid.uuid4())))
    ...
    def test_get_picture_inference_error_not_owner(self):
        ...
        with self.assertRaises(datastore.UserNotOwnerError):
            asyncio.run(datastore.get_picture_inference(self.cursor, str(not_owner_user_id), str(picture_id)))
    ...
    def test_get_picture_blob(self):
        ...
        """
        picture_id = asyncio.run(datastore.upload_picture_unknown(self.cursor, self.user_id, self.pic_encoded,self.container_client))
        blob = asyncio.run(datastore.get_picture_blob(self.cursor, str(self.user_id), self.container_client, str(picture_id)))
    ...
    def test_get_picture_blob_error_user_not_found(self):
        ...
        picture_id = asyncio.run(datastore.upload_picture_unknown(self.cursor, self.user_id, self.pic_encoded,self.container_client))
        with self.assertRaises(datastore.user.UserNotFoundError):
            asyncio.run(datastore.get_picture_blob(self.cursor, str(uuid.uuid4()), self.container_client, str(picture_id)))
    ...
    def test_get_picture_blob_error_connection_error(self):
        ...
        mock_cursor.fetchall.side_effect = Exception("Connection error")
        with self.assertRaises(Exception):
            asyncio.run(datastore.get_picture_blob(mock_cursor, str(self.user_id), self.container_client, str(picture_id)))
    ...
    def test_get_picture_blob_error_picture_set_not_found(self):
        ...
        """
        with self.assertRaises(datastore.picture.PictureNotFoundError):
            asyncio.run(datastore.get_picture_blob(self.cursor, str(self.user_id), self.container_client, str(uuid.uuid4())))
    ...
    def test_get_picture_blob_error_not_owner(self):
        ...
        with self.assertRaises(datastore.UserNotOwnerError):
            asyncio.run(datastore.get_picture_blob(self.cursor, str(not_owner_user_id), self.container_client, str(picture_id)))

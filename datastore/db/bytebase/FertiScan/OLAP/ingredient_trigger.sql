...
CREATE OR REPLACE FUNCTION olap_ingredient_creation()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS ingredient_creation ON "fertiscan_0.0.13".ingredient;
CREATE TRIGGER ingredient_creation
AFTER INSERT ON "fertiscan_0.0.13".ingredient
FOR EACH ROW
EXECUTE FUNCTION olap_ingredient_creation();

CREATE OR REPLACE FUNCTION olap_ingredient_deletion()
RETURNS TRIGGER AS $$
BEGIN
...
DROP TRIGGER IF EXISTS ingredient_deletion ON "fertiscan_0.0.13".ingredient;
CREATE TRIGGER ingredient_deletion
AFTER DELETE ON "fertiscan_0.0.13".ingredient
FOR EACH ROW

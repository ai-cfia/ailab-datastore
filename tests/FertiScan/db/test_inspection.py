...
class test_inspection(unittest.TestCase):
    def setUp(self):
        ...
        self.picture_set_id = picture.new_picture_set(self.cursor,self.picture_set,self.user_id,self.folder_name)
    ...
    def test_new_inspection(self):
        inspection_id = inspection.new_inspection(self.cursor, self.user_id, self.picture_set_id,False)
        self.assertTrue(validator.is_valid_uuid(inspection_id))
    
    def test_is_inspection_verified(self):
        inspection_id = inspection.new_inspection(self.cursor, self.user_id, self.picture_set_id,False)
        inspection_id2 = inspection.new_inspection(self.cursor, self.user_id, self.picture_set_id,True)
    ...
    def test_update_inspection(self):
        inspection_id = inspection.new_inspection(self.cursor, self.user_id, self.picture_set_id,False)
        verified = True
        inspection.update_inspection(self.cursor,inspection_id,verified,None,None,None,None,None,None)
        inspection_data = inspection.get_inspection(self.cursor, inspection_id)
        self.assertEqual(inspection_data[0], verified)
        
    def test_get_all_organization_inspection(self):
        company_id = organization.new_organization(self.cursor, "test-company","test.website.com","0123456789",None)
        inspection_id = inspection.new_inspection(self.cursor, self.user_id, self.picture_set_id,False)
        inspection_id2 = inspection.new_inspection(self.cursor, self.user_id, self.picture_set_id,True)
        inspection.update_inspection(self.cursor,inspection_id=inspection_id,company_id=company_id)
        inspection.update_inspection(self.cursor,inspection_id=inspection_id2,manufacturer_id=company_id)
        inspection_data = inspection.get_all_organization_inspection(self.cursor, company_id)
        self.assertEqual(len(inspection_data), 2)
        self.assertEqual(inspection_data[0][0], inspection_id)
        self.assertEqual(inspection_data[1][0], inspection_id2)